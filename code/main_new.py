...
# !!!====================TODO==================== (3 lines)
# put model to device
# decide optimizer = Adam with learning rate = 3e-4
# decide criterion = L1 loss
# !!!============================================
...
### =========continue training ==============
# Checkpoint = torch.load(r'.\\checkpoint/tasnet_50epoch.pth')
...
def validation():
    ...
    for idx in range(int(len(x_test)/batch_size)):
        ...
        with torch.no_grad():
            ...
            # print('loss size: y_estimate, t_valid', y_estimate.size(), t_valid.size())
            # check y_estimate size and t_valid size
    ...
    print ('Epoch [{}/{}],validation_Loss: {}'.format(epoch+1, epochs_size,np.mean(loss_append)) )        
    model.train()
    return np.mean(loss_append),np.median(SDR_vocal_append),np.median(SDR_music_append)
#%% train
training_loss = []
testing_loss = []
validation_loss = []
...
for epoch in range(epochs_size):
    ...
    for i, (x,t) in enumerate(train_loader):
        ...
        # print(y_estimate.size())    # [10, 1, 16000]
        # y_estimate = y_estimate[0]  # [2, 16000]
        loss = criterion(y_estimate,t.to(device))
        # print('loss size: y_estimate, t', y_estimate.size(), t.size())
        # check y_estimate size and t size
        # print('train size')
        # print(x.size())             # [10, 1, 16000]
        # print(y_estimate.size())    # [2, 16000]
        # print(t.size())             # [10, 2, 16000]
    ...
    plt.legend()
    plt.show()
    writer.add_scalar('Train/loss',train_loss,epoch_now)
    ...
    if (SDR_vocal_DSD100>best_vocal_SDR) & (SDR_music_DSD100>best_music_SDR):
        ...
        checkpoint_path='./checkpoint/tasnet_bestSDR_'+str(epoch_now)+'epoch.pth'
